...
void main() {
  ...
  group('Eventos criados persistem após reinicialização', () {
    test(
      'evento criado pelo organizador reaparece em um novo repositório',
      () async {
        ...
        expect(events.any((event) => event.id == event.id), isTrue);
    ...
    test('dados iniciais não duplicam a cada carga', () async {
      ...
      final seedIds = first.map((event) => event.id).toSet();
    ...
    test(
      'evento com id de seed não é sobrescrito nem duplicado',
      () async {
        final repository = LocalEventRepository();
        ...
        final duplicates = events
            .where((event) => event.id == demoEvents.first.id)
            .toList();
  ...
  group('Pedidos e ingressos persistem após reinicialização', () {
    test(
      'pedido criado continua existindo em um novo repositório',
      () async {
        final repository = LocalPurchaseRepository();

        final saved = await repository.buy(
          user: _user,
          event: demoEvents.first,
        );
    ...
    test(
      'ingresso criado continua existindo em um novo repositório',
      () async {
        final repository = LocalPurchaseRepository();
  ...
  group('Compra não duplica pedidos nem ingressos', () {
    ...
    test(
      'reabrir o aplicativo não reprocessa nem duplica a compra',
      () async {
        final controller = PurchaseController();
        await controller.load();

        final saved = await controller.buy(
          user: _user,
          event: demoEvents.first,
        );
  ...
  group('Tema persiste após reinicialização', () {
    test(
      'preferência salva é carregada por uma nova instância do controlador',
      () async {
        final controller = ThemeController(
          store: SharedPreferencesThemeModeStore(),
        );
  ...
  group('Dados persistidos inválidos não derrubam o aplicativo', () {
    test(
      'repositórios retornam estado seguro com JSON corrompido',
      () async {
        SharedPreferences.setMockInitialValues({
    ...
    test(
      'carregar catálogo com repositório inválido não lança exceção',
      () async {
        SharedPreferences.setMockInitialValues({
        ...
        final event = await repository.eventById(demoEvents.first.id);
  ...
  group('Fluxo completo de compra na interface', () {
    testWidgets(
      'ingresso aparece em Meus Ingressos após comprar',
      (tester) async {
        final router = AppRouter.create();
        final session = SessionController(user: _user);
